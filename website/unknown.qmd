...
plt.figure(figsize=(2.8, 1.8))  # smaller figure
...
for i, country in enumerate(stacked_data['country']):
    if stacked_data['Known'][i] > 15:
        plt.text(stacked_data['Known'][i] / 2, i, f"{stacked_data['Known'][i]:.0f}%", 
                 va='center', ha='center', color='white', fontsize=4.5)
...
plt.title("Data Completeness by Country (%)", fontsize=6.5)

# Smaller legend off to right
plt.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), frameon=False, fontsize=5)
...
plt.subplots_adjust(left=0.28, right=0.82)
...
## Heatmap of Unknown Values by Column and Country
...
- **Relatively complete location data**: Across most countries, location information is more consistently available than other fields.
...
plt.xlabel("Proportion of Gender Data")
plt.title("Gender Data Completeness by Country")
...
<h1>Recommendations <img src="images/light.png" width="60px" style="vertical-align:middle; margin-left:10px;"></h1>
...
summary_table = pd.DataFrame({
    'Country': countries,
    'Total Incidents': [country_totals.get(c, 0) for c in countries],
    'Overall % Unknown': unknown_pct.reindex(countries).values,
    'Avg Unknown Fields': df_filtered.groupby('country')['unknown_count'].mean().reindex(countries).values
})

# Format for display
summary_table['Overall % Unknown'] = summary_table['Overall % Unknown'].round(1)
summary_table['Avg Unknown Fields'] = summary_table['Avg Unknown Fields'].round(1)
summary_table = summary_table.sort_values('Overall % Unknown', ascending=False)

# Display as a styled table
from IPython.display import display, HTML

# Convert to HTML with styling
html_table = summary_table.to_html(index=False, classes='table table-striped table-hover')
styled_table = f"""
